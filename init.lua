...
-- to comment in two inputs.
vim.keymap.set('n', 'gC', function()
  local current_line = vim.fn.line '.'
  if not package.loaded['mini.comment'] then
    require 'mini.comment'
  end
  require('mini.comment').toggle_lines(current_line, current_line)
...
-- remove noice notifications
vim.keymap.set('n', '<leader>n', '<cmd>NoiceDismiss<CR>', { desc = 'Dismiss notice message' })
...
vim.api.nvim_create_autocmd('InsertLeave', {
  ...
  callback = function()
    ...
    -- 2. It's a normal file buffer (buftype is empty)
    -- 3. The buffer has a name (is associated with a file on disk)
    if vim.bo.modifiable and vim.bo.buftype == '' and vim.fn.bufname '%' ~= '' then
      vim.cmd 'update'
    end
  end,
  desc = 'Auto save modifiable file buffers when leaving Insert mode',
})
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          if name:match '^diffview://' then
            -- detach LSP from diffview buffer
  ...
  { -- Collection of various small independent plugins/modules
    ...
    config = function()
      ...
      require('mini.comment').setup {
        ...
        mappings = {
          comment_visual = 'gc', -- if you want gc for visual line comment
        },
      }
